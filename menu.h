    ...
    while (fread(&registro_cc, sizeof(struct REGISTRO_CORRENTE), 1, arquivo_corrente) == 1) 
    {
        if (strcmp(n_c, registro_cc.nome_cc) == 0) 
        {
            ...
            if (arquivo_transacoes != NULL)
            {
                ...
                strcpy(transacao.data, "17/06/2024"); // Aqui você pode usar uma função para obter a data atual
...
void EXTRATO()
{
    ...
    fflush(stdin);
    gets(nome_corrente);    
			//fread Retorna o número de itens completos lidos pela função
